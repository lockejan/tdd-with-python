...
class SendLoginEmailView(TestCase):
    ...
    def test_send_mail_to_address_from_post(self, mock_send_mail):
        ...
        self.assertEqual(from_email, 'superlists@smittie.de')
...
class LoginViewTest(TestCase):
    def test_redirects_to_home_page(self, mock_auth):
        response = self.client.get(f'/accounts/login?token={UUID}')
        self.assertRedirects(response, '/')

    def test_calls_authenticate_with_uid_from_get_request(self, mock_auth):
        self.client.get(f'/accounts/login?token={UUID}')
        self.assertEqual(mock_auth.authenticate.call_args, call(uid=UUID))
